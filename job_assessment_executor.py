...
class JobAssessmentAgentExecutor(AgentExecutor):
    ...
    async def _create_response_parts(self, response_text: str, context_id: str) -> list[Part]:
        ...
        try:
            # Check if response contains label reading questions with images
            if "Looking at [Image:" in response_text:
                # Extract ALL image paths from response text
                import re
                image_paths = re.findall(r'\[Image: ([^\]]+)\]', response_text) or []
                for image_path in image_paths:
                    try:
                        ...
                        if image_path.startswith('label_dataset/'):
                            # Build a hosted URI using BASE_URL or HOST/PORT
                            ...
                            parts.append(
                                Part(
                                    root=FilePart(
                                        file=FileWithUri(uri=image_uri, mime_type=mime_type)
                                    )
                                )
                            )
                        else:
                            ...
                            parts.append(
                                Part(
                                    root=FilePart(
                                        file=FileWithUri(uri=image_path, mime_type=mime_type)
                                    )
                                )
                            )
                    except Exception as e:
